...
class StockPortfolioEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        self.state =  np.append(np.array(self.covs).flatten(),
                      np.array([self.data[tech] for tech in self.tech_indicator_list ]), axis=0) #self.data[tech].values.tolist() 
        self.terminal = False     
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.data = self.df.loc[self.day,:]
            self.covs = self.data['cov_list'] #.values[0]
            self.state =  np.append(np.array(self.covs).flatten(), [self.data[tech] for tech in self.tech_indicator_list ], axis=0)
            # calcualte portfolio return
            # individual stocks' return * weight
            portfolio_return = sum(((self.data.Close / last_day_memory.Close)-1)*weights)
    ...
    def calculate_reward(self, portfolio_value, weights):
    ...
    def _seed(self, seed=None):
        ...
        return [seed]
# # Load data
# financial_data = pd.read_csv('../real_data/stock_data.csv')
# esg_data = pd.read_csv('../real_data/companies_sdgs_vector.csv')

# # Ensure financial data is sorted by date and company
# financial_data = financial_data.sort_values(by=['Date', 'Company']).reset_index(drop=True)
...
# env = StockPortfolioEnv(financial_data, esg_data, user_preferences)
